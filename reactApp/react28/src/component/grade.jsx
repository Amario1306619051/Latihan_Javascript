...
const GradeConverter = () => {
    ...
    return(
        <>
            <input ref={inputElement} type={'number'} onChange={e=>setGrade(parseInt(e.target.value))}/>
            <p>The Converted Grade is : {renderConvertedGrade(grade)}</p>
            <p>Your Previous Grade is : {renderConvertedGrade(previousGrade.current)}</p>
            <p>This component has been rendered : {timesRendered.current} times</p>
            <button onClick={()=>{focusOnInput(inputElement)}}>Focus On The Input</button>
        </>
    )
